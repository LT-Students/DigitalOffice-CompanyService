...
namespace LT.DigitalOffice.CompanyService.Business.UnitTests.Commands.Position
{
  class GetPositionCommandTests
  {
    ...
    public void SetUp()
    {
      ...
      _command = new GetPositionCommand(_repositoryMock.Object, _mapperMock.Object, _accessorMock.Object);
    ...
    {
      _repositoryMock.Setup(x => x.Get(It.IsAny<Guid>())).Throws(new Exception());

      Assert.Throws<Exception>(() => _command.Execute(_positionId));
      _repositoryMock.Verify(repository => repository.Get(It.IsAny<Guid>()), Times.Once);
    }
